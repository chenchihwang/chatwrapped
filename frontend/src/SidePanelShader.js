...
const SidePanelShader = (p) => {
  ...
  p.setup = () => {
    p.createCanvas(p.windowWidth / 3, p.windowHeight, p.WEBGL);
    p.noStroke();
  };
  ...
  p.draw = () => {
    ...
    myShader.setUniform("iMouse", [p.mouseX, p.mouseY]);
    p.shader(myShader);
...
export default SidePanelShader;