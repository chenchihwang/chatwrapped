...
def parse_messages(file_path, target_username):
    ...
    # Average the per-message scores and scale to a 1–10 range.
    if dryness_scores:
        avg_dryness = sum(dryness_scores) / len(dryness_scores)
        final_dryness_score = round(avg_dryness * 9 + 1, 2)  # Scale 0->1 to 1->10.
    else:
        final_dryness_score = None
    
    if humor_scores:
        avg_humor = sum(humor_scores) / len(humor_scores)
        final_humor_score = round(avg_humor * 9 + 1, 2) + 2  # Scale 0->1 to 1->10.
    else:
    ...
    return result  # This is a dictionary