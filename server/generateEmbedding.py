...
def getEmbedding(favorite_label, stats):
    ...
    # === 3. Compute Scaling Factor and Scale Topic Embedding ===
    # We want both blocks (topic embedding and stats vector) to contribute equally.
    scaling_factor = np.sqrt(stats_vector.shape[0] / topic_embedding.shape[0])
    topic_embedding_scaled = topic_embedding * scaling_factor
    
    # === 4. Concatenate the Scaled Topic Embedding with the Stats Vector ===
